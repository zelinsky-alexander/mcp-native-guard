...
Envelope JsonRpcEnvelopeClassifier::classify(std::string_view message) const noexcept {
    ...
    const auto fail = [&](ClassificationError error) noexcept {
        return Envelope{EnvelopeKind::invalid, id_kind, id_json, method, error};
    };
    ...
    if (offset < message.size() && message[offset] == '}') {
        ...
        while (offset < message.size()) {
            ...
            if (!scan_string(message, offset, key) || key.has_escape) {
                return fail(ClassificationError::malformed_json);
            }
            skip_whitespace(message, offset);
            if (offset == message.size() || message[offset++] != ':') {
                return fail(ClassificationError::malformed_json);
            ...
            if (key.contents == "jsonrpc") {
                StringToken version;
                if (saw_jsonrpc) {
                    return fail(ClassificationError::duplicate_member);
                }
                saw_jsonrpc = true;
                if (!scan_string(message, offset, version) || version.has_escape ||
                    version.contents != "2.0") {
                    return fail(ClassificationError::unsupported_jsonrpc_version);
                }
                ...
                if (!scan_string(message, offset, value) || value.has_escape || value.contents.empty()) {
                    return fail(ClassificationError::invalid_envelope);
                }
                ...
                saw_id = true;
                const std::size_t id_start = offset;
                ...
                id_json = message.substr(id_start, offset - id_start);
    ...
    if (saw_method) {
        ...
        return {saw_id ? EnvelopeKind::request : EnvelopeKind::notification, id_kind, id_json, method, {}};
    }
    if (saw_id && (saw_result != saw_error)) {
        return {EnvelopeKind::response, id_kind, id_json, {}, {}};
    }
    return fail(ClassificationError::invalid_envelope);
}
